...
import { inPreloadWindow } from './lib/preload-window.js';
import {
  loadAuth,
...
/* ── ตัวอุ่นข้อมูลเบื้องหลัง ──
 *
 * เดิมเซิร์ฟเวอร์ไม่ดึงอะไรเลยจนกว่าจะมีคนเปิดหน้าเว็บ คนแรกหลังรีสตาร์ท
 * (หรือหลังปล่อยว่างเกิน 5 นาที) จึงต้องนั่งรอโหลดเต็มรอบ ~20 วินาทีถึง ~3 นาที
 * ถ้ามี nginx คั่นและรอบนั้นนานกว่า proxy_read_timeout ผู้ใช้จะได้ **504**
 * ทั้งที่เซิร์ฟเวอร์ยังทำงานอยู่ (เจอจริงบนเซิร์ฟเวอร์ ส.ค. 69)
 *
 * แก้สองชั้น: เสิร์ฟของเก่าไปก่อนแล้วดึงใหม่เบื้องหลัง (getReports) กับอุ่นไว้
 * ล่วงหน้าเป็นรอบ ๆ ตรงนี้ เพื่อให้แม้แต่คนแรกของวันก็ได้ของที่สดพอ
 *
 * **เฉพาะช่วงที่คนทำงานจริง** ตี 3 ไม่มีใครกรอกชีตและไม่มีใครเปิดดู
 * การดึงตอนนั้นคือยิง 137 คำขอไป Google ฟรี ๆ ซึ่งเป็นสิ่งเดียวกับที่คูลดาวน์
 * ปุ่มรีเฟรชถูกออกแบบมาป้องกัน
 *
 * **ทำไมชั่วโมงละครั้ง ไม่ใช่ครึ่งชั่วโมง** — คนกรอกชีตทำงาน 08:00–12:00 ก็จริง
 * แต่ช่วงนั้นคนก็เปิดดูอยู่ด้วย และการเปิดดูแต่ละครั้งกระตุ้นให้ดึงใหม่เบื้องหลัง
 * อยู่แล้ว ความสดที่ได้เพิ่มจากการถี่เป็นสองเท่าจึงน้อย ขณะที่คำขอต่อวันขึ้นจาก
 * 1,918 เป็น 3,699 — และ Google **ไม่ประกาศโควตาของ /export?format=csv ไว้เลย**
 * จึงไม่มีใครรู้เพดานจริง เลือกฝั่งที่เบากว่าและปรับได้ด้วย env ตัวเดียว
 *
 * เริ่ม 07:00 = อุ่นเสร็จก่อนคนแรกมาถึง · จบ 20:00 = ไม่ยิงตอนที่ไม่มีใครใช้ */
const PRELOAD_EVERY_MIN = Number(process.env.PRELOAD_EVERY_MIN ?? 60);
const PRELOAD_HOURS = String(process.env.PRELOAD_HOURS ?? '7-20');
...
async function getReports({ force = false } = {}) {
  ...
  const fresh = age < MEMORY_TTL_MS;
  if (fresh && !force) {
    return { ...cached, meta: { ...cached.meta, cacheHit: true } };
  }

  /* หมดอายุแล้วแต่ยังมีของเก่าอยู่ → **ส่งของเก่าไปก่อน แล้วดึงใหม่เบื้องหลัง**
   *
   * ก่อนหน้านี้ทุกคำขอที่มาหลังแคชหมดอายุจะถูกบล็อกไว้จนโหลดเต็มรอบเสร็จ
   * ซึ่งกินเวลา ~20 วินาทีถึง ~3 นาที นานกว่า proxy_read_timeout ของ nginx
   * (ค่าเริ่มต้น 60 วิ) ผู้ใช้จึงได้ 504 ทั้งที่เซิร์ฟเวอร์ยังทำงานปกติ
   *
   * ของเก่าที่ส่งไปไม่ได้เก่าอย่างเงียบ ๆ — หน้าเว็บขึ้น "อัปเดตเมื่อ N นาทีที่แล้ว"
   * จาก meta.fetchedAt อยู่แล้ว และยังกดปุ่มรีเฟรชเพื่อรอของสดได้ทุกเมื่อ
   *
   * **กดรีเฟรชเอง (force) ต้องรอของจริงเสมอ** ไม่งั้นปุ่มจะเหมือนกดแล้วไม่มีอะไรเกิดขึ้น */
  if (cached && !force) {
    if (!inFlight) revalidate();
    return {
      ...cached,
      meta: { ...cached.meta, cacheHit: true, stale: true, staleMs: age },
    };
  }
...
/**
 * เอาชุดสำรองจากดิสก์ขึ้นหน่วยความจำก่อนเปิดรับคำขอ
 *
 * **นี่คือชิ้นที่ปิดช่องโหว่จริง** — stale-while-revalidate ใช้ได้ต่อเมื่อมี "ของเก่า"
 * ให้เสิร์ฟ แต่หลังรีสตาร์ทหน่วยความจำว่างเปล่า คำขอแรกจึงยังต้องรอรอบเต็ม
 * (วัดได้ 58 วินาที ซึ่งเกิน proxy_read_timeout ค่าเริ่มต้นของ nginx ที่ 60 วิ ไปนิดเดียว
 * และเกินแน่นอนถ้าเซิร์ฟเวอร์ต่อ Google ช้ากว่าเครื่องพัฒนา)
 *
 * cachedAt ตั้งตาม **เวลาที่ชุดสำรองถูกดึงมาจริง** ไม่ใช่ 0 หรือเวลาปัจจุบัน
 *   - ตั้งเป็นเวลาปัจจุบัน = โกหกว่าข้อมูลสด แล้วจะไม่ดึงใหม่ไปอีก 5 นาที
 *   - ตั้งเป็น 0 = staleMs กลายเป็นพันล้านวินาที ซึ่งเป็นตัวเลขที่ไม่มีความหมาย
 *     และคนที่เอาไปใช้ทีหลังจะได้ขยะโดยไม่รู้ตัว
 *   - ตั้งตามเวลาจริง = ทั้ง fresh, staleMs และ "อัปเดตเมื่อ …" บนหน้าเว็บตรงกันหมด
 *     และถ้าเพิ่งรีสตาร์ทหลังโหลดเสร็จไม่นาน ก็ถือว่าสดจริง ไม่ต้องดึงซ้ำ
 */
async function primeFromSnapshot() {
  if (cached) return;
  const snap = await loadFromSnapshot();
  if (!snap || cached) return;
  cached = snap;
  cachedAt = Date.parse(snap.meta?.fetchedAt ?? '') || 0;
  console.log(`  ชุดสำรองจากดิสก์: เสิร์ฟได้ทันที (ข้อมูล ณ ${snap.meta?.fetchedAt ?? '?'})`);
}

/**
 * ดึงใหม่เบื้องหลังโดยไม่ให้ใครรอ — error ถูกกลืนที่นี่เพราะไม่มีผู้เรียกให้โยนกลับ
 *
 * @param {object} [o]
 * @param {number} [o.skipIfFresherThanMs] ข้ามถ้าข้อมูลยังใหม่กว่านี้
 *   ใช้กับรอบที่มาจาก **ตัวตั้งเวลา** เท่านั้น เพราะถ้าเพิ่งมีคนเปิดดูไปเมื่อนาทีที่แล้ว
 *   ข้อมูลก็ถูกดึงใหม่ไปแล้ว การดึงซ้ำคือยิง 137 คำขอไป Google โดยไม่ได้อะไรเพิ่ม
 *   ส่วนรอบที่มาจากคำขอผู้ใช้ไม่ส่งค่านี้ — มันมีเงื่อนไข TTL คุมของตัวเองอยู่แล้ว
 */
function revalidate({ skipIfFresherThanMs = 0 } = {}) {
  if (skipIfFresherThanMs > 0 && cached && Date.now() - cachedAt < skipIfFresherThanMs) return;
  getReports({ force: true }).catch((err) =>
    console.warn('[preload] ดึงเบื้องหลังไม่สำเร็จ ยังเสิร์ฟชุดเดิมต่อ:', err.message)
  );
}

/**
 * อุ่นข้อมูลเป็นรอบ ๆ เพื่อให้ไม่มีใครต้องเจอการโหลดเต็มรอบ
 *
 * ใช้ unref() เพื่อไม่ให้ตัวตั้งเวลาค้างโปรเซสไว้ตอนสั่งปิด
 */
function startPreloader() {
  if (!(PRELOAD_EVERY_MIN > 0)) {
    console.log('  อุ่นข้อมูลอัตโนมัติ: ปิดอยู่ (PRELOAD_EVERY_MIN=0)');
    return;
  }
  console.log(
    `  อุ่นข้อมูลอัตโนมัติ: ทุก ${PRELOAD_EVERY_MIN} นาที ช่วง ${PRELOAD_HOURS} น.`
  );

  // อุ่นทันทีที่เปิดเซิร์ฟเวอร์ ไม่ต้องรอครบรอบแรก — คนแรกหลัง deploy จะได้ไม่ต้องรอ
  revalidate();

  /* ชีตวัสดุอุ่น **ครั้งเดียวตอนสตาร์ท ไม่เข้ารอบ**
   *
   * มันเป็น 130 คำขอต่อรอบ ถ้าเอาเข้ารอบชั่วโมงละครั้งจะเพิ่มอีก ~1,800 คำขอ/วัน
   * โดยที่ TTL 15 นาทีของมันเอง กับการที่เบราว์เซอร์ยิงเองหลังหน้าแรกวาดเสร็จ
   * ครอบให้อยู่แล้ว — ที่ยังต้องอุ่นตรงนี้เพราะหลัง deploy ใหม่ ๆ คนที่เปิด
   * #/supply ตรง ๆ จะเจอแคชเย็นซึ่งเป็นสถานการณ์เดียวกับที่กำลังแก้อยู่ */
  getLazySource('supplyLog').catch((err) =>
    console.warn('[preload] อุ่นชีตวัสดุไม่สำเร็จ:', err.message)
  );

  setInterval(() => {
    if (!inPreloadWindow(PRELOAD_HOURS)) return;
    // ครึ่งรอบ — เพิ่งมีคนเปิดดูไปไม่นานก็ไม่ต้องยิงซ้ำ
    revalidate({ skipIfFresherThanMs: (PRELOAD_EVERY_MIN * 60_000) / 2 });
  }, PRELOAD_EVERY_MIN * 60_000).unref();
}
...
async function start() {
  ...
  /* อ่านชุดสำรองขึ้นมา **ก่อน** เปิดรับคำขอ ไม่ใช่หลัง
   * ถ้าทำหลัง จะมีช่องให้คำขอแรกเข้ามาตอนที่ยังไม่มีของเก่าให้เสิร์ฟ
   * แล้วมันจะไปรอรอบเต็มพอดี ซึ่งเป็นอาการเดียวกับที่กำลังแก้อยู่ */
  await primeFromSnapshot();

  server.listen(PORT, HOST, () => {
    console.log(`\n  เปิดที่  http://${HOST}:${PORT}\n`);
    /* เรียกหลัง listen เสมอ ไม่ใช่ก่อน — ไม่งั้นเซิร์ฟเวอร์จะยังไม่รับคำขอ
     * ตลอดเวลาที่ดึงข้อมูลรอบแรก ซึ่งเป็นอาการเดียวกับที่กำลังแก้อยู่พอดี */
    startPreloader();
  });
}
