...
export function buildDataContext(payload, supplyPayload = null) {
  ...
  if (fin?.available) {
    ...
    /* รายได้รายลูกค้า (แท็บ Revenue) — ตอบ "ลูกค้าไหนซื้อมากสุด" และ "ในประเทศกี่ %" ได้
     * ใส่ทุกราย เพราะมีแค่ ~10 ราย ถ้าตัดเหลือ top 5 คำถามถึงรายเล็กจะถูกตอบว่าไม่มีข้อมูล */
    const split = fin.revenueSplit;
    if (split?.available) {
      const segName = { domestic: 'ในประเทศ', export: 'ต่างประเทศ' };
      const segOf = (s) => segName[s] ?? s ?? 'ไม่ระบุกลุ่ม';
      out.push(`- รายได้แยกกลุ่มลูกค้า (แท็บ "Revenue" ช่วงเดียวกับยอดข้างบน รวม ${n(split.total)}):`);
      for (const s of split.segments) {
        out.push(`  - ${segOf(s.key)}: ${n(s.amount)} (${n(s.share, 1)}%) · ${s.customers} ราย`);
      }
      out.push('- รายได้รายลูกค้า (ลูกค้า | กลุ่ม | รายได้ | สัดส่วน | เดือนที่มียอด):');
      for (const c of split.customers) {
        out.push(
          `  - ${c.customer} | ${segOf(c.segment)} | ${n(c.amount)} | ${n(c.share, 1)}% | ${c.monthsWithValue} เดือน (ล่าสุด ${c.lastMonth ?? '—'})`
        );
      }
    } else {
      out.push('- ชีตต้นทุนยังไม่มีแท็บ "Revenue" รายลูกค้า จึงแยกรายได้รายลูกค้าไม่ได้');
    }

    /* ต้นทุนต่อกรัม — ตามกติกาในแท็บ "ต้นทุนต่อกรัม" เท่านั้น (รวม Cost ÷ Gram ของเดือนเดียวกัน) */
    const pg = fin.perGram;
    if (pg?.available) {
      const d2 = (v) => (v === null || v === undefined ? '—' : Number(v).toFixed(2));
      out.push(
        `- ต้นทุนต่อกรัม (แท็บ "ต้นทุนต่อกรัม" · คิดใหม่จาก รวม Cost ทั้งหมด ÷ Gram ของเดือนเดียวกัน): ` +
          `สะสม ${d2(pg.totals.costPerGram)} บาท/กรัม จาก ${pg.totals.monthsWithGrams} เดือนที่มีกรัม ` +
          `(ต้นทุน ${n(pg.totals.cost)} ÷ ${n(pg.totals.grams)} g)` +
          (pg.budget ? ` · งบประมาณ (${pg.budget.label ?? ''}) ${d2(pg.budget.costPerGram)} บาท/กรัม` : '')
      );
      out.push('- ต้นทุนต่อกรัมรายเดือน (เดือน | กรัม | ต้นทุน | บาท/กรัม):');
      for (const m of pg.months) {
        out.push(`  - ${m.month} | ${n(m.grams)} | ${n(m.cost)} | ${d2(m.costPerGram)}`);
      }
      out.push('  (เดือนที่บาท/กรัมเป็น — คือมีต้นทุนแต่ยังไม่มีกรัม ห้ามยกต้นทุนไปรวมกับเดือนอื่น)');
    } else {
      out.push('- ยังไม่มีแท็บ "ต้นทุนต่อกรัม" ที่อ่านได้ จึงยังตอบต้นทุนต่อกรัมไม่ได้');
    }
    out.push('');
  } else {
...
12. เงินในระบบมาจากสามที่ ซึ่ง **คนละขอบเขตกัน ห้ามเอามาบวกกัน**
   ...
   - **รายได้รายลูกค้า และสัดส่วนในประเทศ/ต่างประเทศ** → แท็บ "Revenue" ของชีตต้นทุน
     ตอบได้ว่าลูกค้าไหนซื้อเท่าไร เดือนไหน กลุ่มไหนกี่ % — แต่แท็บนี้ **ไม่มีสายพันธุ์
     ไม่มีน้ำหนัก** จึงบอกไม่ได้ว่าลูกค้ารายนั้นซื้อดอกอะไรเป็นเงินเท่าไร
   - **รายงานการขายดอกยังไม่มีคอลัมน์ราคา** จึงแยกรายได้รายสายพันธุ์/ขนาด
     หรือคิดราคาขายเฉลี่ยต่อกรัม (ASP) **ไม่ได้** ถ้าถูกถามให้ตอบตรง ๆ ว่าชีตยังไม่มีราคา
     **ห้ามเอารายได้ (รวมหรือรายลูกค้า) ไปหารด้วยกรัมที่ขายเพื่อเดา ASP เด็ดขาด**
     เพราะรายได้ก้อนนั้นไม่ได้ผูกกับรายการขายรายบรรทัด — แม้ชื่อลูกค้าจะตรงกันสองชีตก็ตาม
...
14. **ต้นทุนต่อกรัมตอบได้เฉพาะตัวเลขในหัวข้อ "ต้นทุนต่อกรัม" ของบริบท** ซึ่งคิดตามกติกา
   ในแท็บ "ต้นทุนต่อกรัม" ของชีตต้นทุน (รวม Cost ทั้งหมด ÷ Gram ที่คนกรอกไว้ในเดือนเดียวกัน)
   **ห้ามเอาต้นทุนจากงบสรุปไปหารผลผลิตจากรายงานทริม/ต่อครอปเอง** เพราะต้นทุนของเดือนหนึ่ง
   เป็นของครอปที่เก็บเกี่ยวอีกเดือนหนึ่ง กรัมในแท็บนั้นคือตัวที่คนตัดสินแล้วว่าผูกกับต้นทุนเดือนไหน
   เดือนที่มีต้นทุนแต่ยังไม่มีกรัมให้ตอบว่ายังคิดไม่ได้ ห้ามยกต้นทุนไปรวมกับเดือนอื่น`;
