...
describe('หน่วยราคา · หน่วยสต๊อก · หน่วยซื้อ', () => {
  ...
  test('จำนวนที่ขอซื้อคิดจากแท็บ log เอง ไม่ใช้ช่อง "จำนวนสั่งซื้อ" ในตารางสั่งของ', () => {
    /* เคสจริงที่ผู้ใช้จับได้ ส.ค. 69 — ฟองน้ำพร้อมใยขัด (สก็อตไบร์ท)
     *
     * ตารางสั่งของรายเดือนเขียน `จำนวนสั่งซื้อ 12` ค้างไว้ (พร้อมยอดคงเหลือเก่า 10)
     ...
     * — 12 แพ็ค (14 ชิ้น/แพ็ค = 168 ชิ้น) คือของใช้ 6 เดือน */
    const kpi = buildKpi(
      {
        supplyLog: {
          key: 'supplyLog',
              ...
              tabs: [
                {
                  ...
                  rows: [
                    ['', 'รายการ', 'หน่วย', 'คงเหลือ', 'จำนวนสั่งซื้อ', 'ราคา / @', 'รวม', 'วันสั่ง', 'ล่าสุด', 'ระยะเวลา'],
                    // คงเหลือ 10 ในตารางนี้ขัดกับ log ที่เหลือจริง 1 — เลข 12 จึงมาจาก
                    // สถานการณ์คนละอันกับตอนนี้
                    ['32', 'ฟองน้ำพร้อมใยขัด-สก็อตไบร์ท (14 ชิ้น/แพค)', 'แพค', '10', '12', '155', '1860', '25', '', '1 เดือน'],
                  ],
                },
                {
                  gid: '1',
                  name: '32.ฟองน้ำพร้อมใยขัด (สก็อตไบร์ท)',
                  rows: [
                    ['แบบฟอร์มเบิกของและของคงเหลือ (33)', '', '', '', '', '', '', 'ราคา/แพ็ค'],
                    ['', '', '', '', '', '', '', '189'],
                    ['ฟองน้ำพร้อมใยขัด-สก็อตไบร์ท (14 ชิ้น/แพค): ใช้ 2 แพ็ค/เดือน สั่งครั้งละ 2 แพ็ค Lead time 7 days'],
                    ['วัน/เดือน/ปี ', 'จำนวน\nรับของ', 'จำนวนเบิก', 'จำนวน\nคงเหลือ', 'หน่วย', 'ขั้นต่ำ', 'Index'],
                    ['01/08/2569', '', '', '1', 'แพ็ค', '1', '0'],
                  ],
                },
              ],
              today: '2026-08-20',
    ...
    const row = kpi.needsReorder.find((r) => /สก็อตไบร์ท/.test(r.item));
    assert.ok(row, 'คงเหลือ 1 = ขั้นต่ำ 1 → Index 0 ต้องเข้าเกณฑ์ Index ≤ 0');
    assert.equal(row.orderQty, 12, 'ยังอ่านเลขจากตารางมาเก็บไว้ได้ แต่ต้องไม่เอาไปใช้');
    assert.notEqual(row.suggestedPacks, 12, 'ห้ามเชื่อเลขที่ค้างอยู่ในตารางสั่งของ');
    assert.equal(row.suggestedPacks, 2, 'ต้องปัดขึ้นตาม "สั่งครั้งละ 2 แพ็ค" ในหมายเหตุ');
    assert.equal(row.amount, 378, 'ห้ามได้ 2,268 ซึ่งเป็นตัวเลขที่ผู้ใช้ทักท้วง');
  });
...
describe('แท็บปุ๋ยที่แยกเป็นสูตร A/B', () => {
  /* เคสจริง ส.ค. 69 — ผู้ใช้เปลี่ยนมาใช้ปุ๋ยน้ำสองส่วน แท็บจึงกลายเป็น
   * `Bloom A` · `Bloom B` · `Grow A` · `Grow B` (แท็บ `Core` ถูกเปลี่ยนชื่อเป็น `Bloom B`
   * และ `Cleanse` เป็น `Grow B` โดยใช้ gid เดิม)
   *
   * ชื่อพวกนี้ไม่ตรงลิสต์ปุ๋ยแบบเป๊ะ ๆ และไม่มีเลขนำหน้า จึงถูกข้ามทั้งแท็บ
   * ทั้ง 4 รายการหายจากมูลค่าสต๊อกและจากตารางของที่ต้องสั่งซื้อ ทั้งที่ต่ำกว่าขั้นต่ำทุกอัน */
  const nutrientTab = (name, rows) => ({
    gid: name,
    name,
    rows: [
      ['แบบฟอร์มเบิกของและของคงเหลือ (33)', '', '', '', '', '', '', 'ราคา/ถัง'],
      ['', '', '', '', '', '', '', '1037.5'],
      [name],
      ['วัน/เดือน/ปี ', 'จำนวน\nรับของ', 'จำนวนเบิก', 'จำนวน\nคงเหลือ', 'หน่วย', 'ขั้นต่ำ', 'Index'],
      ...rows,
  ...
  test('ชื่อที่ลงท้ายด้วยตัวอักษรเดี่ยวต้องยังนับเป็นแท็บปุ๋ย', () => {
    const out = parseSupplyLog({
      tabs: ['Bloom A', 'Bloom B', 'Grow A', 'Grow B', 'Bloom A/B'].map((n) =>
        nutrientTab(n, [['01/08/2569', '', '', '9', 'ถัง', '20', '-11']])
      ),
      today: '2026-08-21',
    });
    assert.equal(
      out.tabs.filter((t) => t.skipped).length,
      0,
      'ถูกข้ามเมื่อไร ทั้ง 4 รายการจะหายจากมูลค่าสต๊อกและตารางสั่งซื้อโดยไม่มีตัวเลขไหนขยับ'
    );
  ...
  test('ตัดหางเฉพาะตัวอักษรเดี่ยวที่มีช่องว่างคั่น ห้ามกินตัวท้ายของชื่อจริง', () => {
    const out = parseSupplyLog({
      tabs: ['COCO', 'CO2', 'pH Up', 'CaMg', 'Fade'].map((n) =>
        nutrientTab(n, [['01/08/2569', '', '', '5', 'ถุง', '2', '3']])
      ),
      today: '2026-08-21',
    });
    // `coco` ห้ามถูกตัด `o` ท้ายจนเหลือ `coc` แล้วหลุดลิสต์ · `pH Up` ห้ามถูกตัด `up`
    assert.equal(out.tabs.filter((t) => t.skipped).length, 0);
  });

  test('ชื่อที่ไม่ใช่ปุ๋ยและไม่มีเลขนำหน้า ยังต้องถูกข้ามพร้อมเตือนเหมือนเดิม', () => {
    const out = parseSupplyLog({
      tabs: [nutrientTab('สรุปยอดรวม', [['01/08/2569', '', '', '5', 'ถุง', '2', '3']])],
      today: '2026-08-21',
    });
