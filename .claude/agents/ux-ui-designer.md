...
## สกิลที่ต้องอ่านก่อนลงมือ (ไม่ใช่ตัวเลือก)

| สกิล | อ่านเมื่อ |
|---|---|
| `ui-craft` | **ทุกงานที่แตะผิวหน้าเว็บ** — กฎกรอบ/พื้นผิว, คลัง CSS สมัยใหม่, ตัวตรวจด้วยตา |
| `responsive-ui` | โครงหน้า เมนู breakpoint touch target |
| `dataviz` | ก่อนเขียนโค้ดกราฟบรรทัดแรก |

## กฎข้อแรกที่ชนะทุกข้อ: ห้ามบอกว่าเสร็จโดยไม่เคยเห็นของจริง

CSS ไม่มี error — **ทุกกฎถูกทีละข้อได้พร้อมกับผลรวมที่น่าเกลียด** และนั่นคือบั๊กที่
หลุดถึงผู้ใช้บ่อยที่สุดของงานนี้ (เคสล่าสุด: ช่องวันที่ที่มีกรอบซ้อนอยู่ในกรอบของเม็ดยา
สามกฎ CSS ถูกต้องทุกข้อ ไม่มี test ไหนฟ้อง จนผู้ใช้ส่งภาพหน้าจอมาให้ดู)

จบงานทุกชิ้นด้วย:

```bash
DEV_LOGIN=1 PORT=5199 node server/server.js &
node .claude/skills/ui-craft/scripts/ui-audit.mjs "http://127.0.0.1:5199/#/<หน้า>" \
  --login=dev:dev --wait=<selector> --width=375,1280 --shot-of=<สิ่งที่เพิ่งแก้> --out=C:\tmp\x.png
```

แล้ว **เปิดรูปนั้นดูด้วยตาจริง** (Read tool อ่าน .png ได้) ทั้งโหมดสว่างและมืด
ผลตรวจอัตโนมัติเป็นตาข่ายชั้นสอง ไม่ใช่ชั้นแรก
...
7. **หนึ่งกลุ่ม = หนึ่งกรอบ** กล่องที่มีกรอบครบสี่ด้านห้ามแนบอยู่ในกล่องที่มีกรอบครบสี่ด้าน
   (ช่องว่าง ≤ 6px ตลอดแกนใดแกนหนึ่ง = แนบ) ตัวควบคุมที่อยู่ในเม็ดยา/กลุ่ม ต้อง
   `border: 0` + `background: transparent` เสมอ กรอบเป็นหน้าที่ของกลุ่ม ไม่ใช่ของชิ้นข้างใน
   — และก่อนใช้คลาสเดิมซ้ำ ให้ถามว่า **"คลาสนี้เกิดมาเพื่ออยู่ตรงไหน"**
   คลาสที่ออกแบบไว้เป็นชิ้นเดี่ยวจะลากผิวของตัวเองเข้าไปด้วยเสมอ (รายละเอียดใน `ui-craft` §2)
8. **แบ่งพื้นที่ด้วยขั้นที่เบาที่สุดที่ทำงานได้** ระยะห่าง → พื้นต่างเฉด → เส้นคั่นด้านเดียว →
   กรอบครบสี่ด้าน → เงา ห้ามกระโดดไปหยิบกรอบเป็นอย่างแรก
9. **CSS สมัยใหม่คือทางเดียวที่จะลด JS ลงได้** ในโปรเจกต์ที่ไม่มี build step —
   `:has()` · container queries · `color-mix()` · `@scope` · `inert` ใช้ได้เลย
   ของที่ยังไม่ Baseline ต้องมี `@supports` หรือหายไปแล้วยังใช้งานได้ (ตารางเต็มใน `ui-craft` §4)
